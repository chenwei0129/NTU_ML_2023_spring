...
class QA_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            ...
            shift = random.uniform(1.5, 3.0)
            mid_float = (answer_start_token + answer_end_token) // shift
            mid = int(mid_float)
...
def evaluate(data, output, paragraphs, token):
    ##### TODO: Postprocessing #####
    ...
    for k in range(num_of_windows):
        ...
        #token_list = ['[SEP]', '[PAD]', '[CLS]']
        
        # Replace answer if calculated probability is larger than previous windows
        if prob > max_prob and start_index <= end_index and (end_index-start_index)<=30:
            temp = tokenizer.decode(data[0][0][k][start_index : end_index + 1])
            if '[SEP]' not in temp and '[PAD]' not in temp and '[CLS]' not in temp and temp != '':
                max_prob = prob
        ...
        if '[UNK]' in answer:
            token_split = token[250*k:]
            num = 0
            for word in token[:250*k]:
                if word == '[UNK]' or word == '[CLS]' or word == '[SEP]':
                    num = num + 1
                else:
                    for w in word:
                        if w != '#':
                            num = num + 1
            paragraphs_split = paragraphs[num:]
            c = 0
            while(True):
                ttt = tokenizer.decode(data[0][0][k][c])
                if ttt == '[SEP]':
                    break
                c = c + 1
            #real_answer = temp[start_index-c-1 : end_index-c-1 + 1]
            start = start_index - c - 1
            end = end_index - c - 1
            word_num_start = 0
            for word in token_split[:start]:
                if word == '[UNK]' or word == '[CLS]' or word == '[SEP]':
                    word_num_start = word_num_start + 1
                else:
                    for w in word:
                        if w != '#':
                            word_num_start = word_num_start + 1
            word_num_end = 0
            for word in token_split[:end]:
                if word == '[UNK]' or word == '[CLS]' or word == '[SEP]':
                    word_num_end = word_num_end + 1
                else:
                    for w in word:
                        if w != '#':
                            word_num_end = word_num_end + 1
            real_answer1 = paragraphs_split[word_num_start : word_num_end + 1]
            #print(k)
            #print(answer)
            #print(real_answer)
            #print(real_answer1)
